...
def draw_rectangles(img, rec_data):
    
    for o_data in rec_data:
        name = o_data[0].decode("utf-8")
        (x_cord, y_cord, width, height) = o_data[2]
        x_cord = int(x_cord)
        y_cord = int(y_cord)
        width = int(width)
        height = int(height)
        # top_left = (x_cord, y_cord+height)
        # bottom_right = (x_cord+width, y_cord)
        # top_left = (x_cord, y_cord)
        # bottom_right = (x_cord+width, y_cord-height)

        top_left = (x_cord - int(width/2), y_cord + int(height/2))
        bottom_right = (x_cord + int(width/2), y_cord - int(height/2))

        r_color = (random.randint(0,255), random.randint(0,255), random.randint(0,255))
        cv2.rectangle(img, top_left, bottom_right, r_color, 3)
        cv2.putText(img, name, (x_cord, y_cord), cv2.FONT_HERSHEY_PLAIN, 2.5, r_color, 3)
